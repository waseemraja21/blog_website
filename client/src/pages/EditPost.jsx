...
export default function EditPost() {
  ...
  const editPost=async(ev)=>{
    ...
    try{
      const response = await axios.patch(`${process.env.REACT_APP_BASE_URL}/posts/${id}/edit`, postData, {withCredentials: true, headers: {Authorization: `Bearer ${token}`}});
      ...
      <div className='container'>
       ...
       {error && <p className='error'>{error}</p>}
        <form className='form create_post-form' onSubmit={editPost}>
          <input type='text' placeholder='Title' value={title} onChange={ev=>setTitle(ev.target.value)} autoFocus/>
